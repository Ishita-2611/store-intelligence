...
The main reason was reproducibility. The local environment did not already include `ultralytics` or `torch`, and the challenge needs to run with minimal setup. Relying on model downloads or GPU acceleration as a hard requirement would create a fragile demo. The current baseline works with normal Python dependencies and produces the required schema from the provided challenge resources or from raw CCTV clips when they are available. It also keeps confidence values visible rather than dropping weak detections, which helps the API and reviewer understand uncertainty.
...
For billing conversion, I used the challenge's time-window idea but adapted it to the available data. The pipeline emits `BILLING_QUEUE_JOIN` for every visitor entering a billing zone, including the first person in the queue, and can emit `BILLING_QUEUE_ABANDON` when a visitor leaves the billing zone and no POS transaction follows within five minutes. Because the POS file has no customer identity, the correlation is store-and-time-window based: one transaction in the next five minutes can plausibly represent any recent billing visitor. If no POS reference rows exist for that store, the pipeline withholds abandonment instead of manufacturing a negative outcome. That is explainable and aligned with the anonymized data, but it can underreport abandonment in busy periods. The API treats billing visitors minus abandoners as purchase-like converted visitors, giving a transparent session-level conversion signal from the available data.

Store 2 includes two entry cameras. I made `ENTRY_1` the authoritative source for customer entry counts and kept `ENTRY_2` as observation-only zone evidence. That avoids double-counting the same doorway traffic while still allowing the secondary angle to contribute movement context.
